...
1. Search for `radiohead` (inside single quotations) to capture the text domain.
2. Search for `radiohead_` to capture all the function names.
3. Search for ` _s` (with a space before it) to capture DocBlocks.
4. Search for `radiohead-` to capture prefixed handles.

OR

* Search for: `radiohead` and replace with: `'megatherium'`
* Search for: `radiohead_` and replace with: `megatherium_`
* Search for: <code>&nbsp;radiohead</code> and replace with: <code>&nbsp;Megatherium</code>
* Search for: `radiohead-` and replace with: `megatherium-`
